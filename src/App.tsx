...
function App() {
    ...
    const [heighestStreak, setHighestStreak] = useState(0)
    const [currentStreak, setCurrentStreak] = useState(0)

    const generateColour = () => {
        let rn = () => Math.floor(Math.random() * 256).toString(16).padEnd(2, "0")
        return `#${rn()}${rn()}${rn()}`
    }
    ...
    const verifySelection = (selectedIndex: number) => {
        ...
        if (success) {
            ...
            if (currentStreak > heighestStreak) setHighestStreak(currentStreak)
